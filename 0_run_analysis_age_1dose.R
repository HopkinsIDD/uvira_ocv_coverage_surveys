...
# 
# # Export
# ggsave("decline_stan_overall.pdf", plot = plot_overall, width = 10, height = 7)
...
# 
# plot_age
# ggsave("decline_stan_age.pdf", plot = plot_age, width = 10, height = 7)
...
#   geom_point(data = coverage_master, aes(x = time_since, y = mean_coverage), size=2, alpha=0.7) +
#   geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci), alpha = 0.3, fill = "gray", linewidth=0.1) +
#   geom_line(aes(x = time, y = weighted_average)) +
#   labs(x = "Months post-vaccination", y = "At least 1-dose coverage", title = "") +
#   geom_vline(xintercept = data_list$t[1], linetype = "dashed") +
#   geom_vline(xintercept = data_list$t[2], linetype = "dashed") +
#   geom_vline(xintercept = data_list$t[3], linetype = "dashed") + theme_minimal() + ylim(0, 1.0) + theme(axis.title.x = element_text(size = 4),  # Change size of x-axis label
#                                                                                                         axis.title.y = element_text(size = 4),
#                                                                                                         axis.text.x = element_text(size = 4),                                                                                                   
#                                                                                                         axis.text.y = element_text(size = 4)) # Change size of x-axis tick mark 
...
# Export
ggsave("decline_stan_points.pdf", plot = plot_points, width = 16, height = 7)
...
#   geom_vline(xintercept = data_list$t[2], linetype = "dashed") +
#   geom_vline(xintercept = data_list$t[3], linetype = "dashed") +
#   scale_color_discrete(name = "Age at \ntime of \ncampaign", labels = c("1-4", "5-9", "10-19", "20-34", "35-49", "50+")) +
#   scale_size_continuous(name = "Sampled \npopulation \nsize", range = c(1, 3)) +  
...
#                           legend.key.size = unit(0.1, "lines"),  # Adjust key size
#                           legend.text = element_text(size = 2),  # Adjust text size
#                           legend.title = element_text(size = 2),  # Adjust title size 
#                           axis.title.x = element_text(size = 4),  # Change size of x-axis label
#                           axis.title.y = element_text(size = 4),
#                           axis.text.x = element_text(size = 4),
#                           axis.text.y = element_text(size = 4),
#                           legend.spacing = unit(0.0001, "cm")  # Adjust space between legends here
#                           ) + ylim(0, 1.0) + guides(
#                             color = guide_legend(order = 2, nrow = 6, override.aes = list(size = 0.5)),  # Legend for color in 1 row
#                             size = guide_legend(order = 1, nrow = 2)    # Legend for size in 1 row
#                           )
# 
...
ggsave("decline_stan_overall_age.pdf", plot = full, width = 16, height = 7)
# ggsave(filename = "decline_stan_overall_age.tiff", last_plot(), bg="white", path = here::here(), width = 90, height = 90, device='tiff', dpi=700, units="mm")
...
overall_decline <- overall_decline %>% mutate(overall_estimate = 0.1642)
...
# Another way of estimating confidence interval
...
#   # Sample with replacement from the individual estimates
#   bootstrap_sample <- sample(mean_estimates, size = length(mean_estimates), replace = TRUE)
#   
#   # Calculate the bootstrap estimate for overall decline rate
#   bootstrap_estimate <- sum(bootstrap_sample * population_sizes) / sum(population_sizes)
#   
#   # Store the estimate in the vector
#   bootstrap_estimates[i] <- bootstrap_estimate
# }
# 
# # Calculate the 95% confidence interval from the bootstrap estimates
# confidence_interval <- quantile(bootstrap_estimates, c(0.025, 0.975))
# 
# # Print the results
# cat("Estimated Overall Decline Rate:", overall_decline_rate_estimate, "\n")
# cat("95% Confidence Interval for Overall Decline Rate:", confidence_interval, "\n")
# 
# # Define the estimated overall decline rate and confidence interval
# estimated_decline_rate <- overall_decline_rate_estimate
